/****************************************************************************************
 Extension Name: EXT050MI.AddNewDel
...
public class AddNewDelZ extends ExtendM3Transaction {
  ...
  public AddNewDelZ(MIAPI mi, DatabaseAPI database, LoggerAPI logger, ProgramAPI program, UtilityAPI utility, MICallerAPI miCaller, IonAPI ion, ExceptionAPI exception) {
    this.mi = mi
    this.ion = ion
  ...
  Closure<?> ext057ReaderLineTransfer = { DBContainer ext057Result ->
    ...
    if (getoolineData(ridn, ridl, ridx)) {
      ...
      if (alqt != 0)
        alqt = convertQty(this.oolineItno, alun, 0, alqt)
  ...
  public boolean getoolineData(String orno, String ponr, String posx) {
    ...
    if (oolinequery.read(oolineRequest)) {
      ...
      this.oolineOrno = oolineRequest.get("OBORNO")
      ...
      this.oolineItno = oolineRequest.get("OBITNO")
  ...
  Closure<?> ext057ReaderPrepareTransfer = { DBContainer ext057Result ->
    ...
    if (getoolineData(ext057Result.getString("EXORNO"), "" + ext057Result.get("EXPONR"), "" + ext057Result.get("EXPOSX"))) {
      ...
      ext057Alqt = convertQty(this.oolineItno, alun, 0, alqt)
