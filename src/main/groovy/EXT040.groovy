/****************************************************************************************
 Extension Name: EXT040
 Type: ExtendM3Transaction
 Script Author: RENARN
 Date: 2023-03-09
 Description:
 * Generates the calendar file and associated files

 Revision History:
 Name        Date        Version  Description of Changes
 RENARN      2023-03-09  1.0.1    COMX02 - Cadencier
 RENARN      2023-11-22  1.0.2    New columns added. Columns heading rectified
 PATBEA      2024-03-27  1.0.3    Correction Key to read table CCUCON. CCEMRE missing
 PATBEA      2024-04-02  1.0.4    Execute EXT041 (ECOM_PROD_STO File) when assortiment is complet
 YVOYOU      2024-04-12  1.0.5    Addition of delete EXT045,EXT046 and no line in calendar if sapr < pupr
 FLEBARS     2024-04-19  1.0.6    Chemin fichier dans docnumber à rendre dynamique **ATTENTION NE PAS FAIRE VALIDER**
 FLEBARS     2024-05-24  1.0.7    Alcool et contrôle prix
 FLEBARS     2024-08-06  1.0.8    Évolution 20, 52, 56
...
public class EXT040 extends ExtendM3Batch {
  ...
  private performActualJob(Optional<String> data) {
    ...
    Closure<?> ext042Reader = { DBContainer EXT042_Type ->
      ...
      assortment = EXT042_Type.get("EXASCD") as String

    }
    if (!ext042Query.readAll(ext042Request, 3, 1, ext042Reader)) {
  ...
  public void retrieveSalesPrice() {
    ...
    DBAction oprbasQuery = database.table("OPRBAS").index("00").matching(oprbasExpression).selection("ODVFDT", "ODSAPR", "ODSPUN").reverse().build()
    DBContainer oprbasRequest = oprbasQuery.getContainer()
    oprbasRequest.set("ODCONO", currentCompany)
    oprbasRequest.set("ODPRRF", prrf)
    oprbasQuery.readAll(oprbasRequest, 2, 1, oprbasReader)
  }
  /**
  ...
  public void createCustomerEmailFile() {
    ...
    if (inAllContacts == 1) {
      ...
      if (!ext043Query.readAll(ext043Reqquest, 3, 100, ext043Reader)) {
      }
    }
  ...
  public void createInternalEmailFile() {
    ...
    if (inSchedule == 1) {
      ...
      if (!ext044Query.readAll(ext044Request, 3, 100, ext044Reader)) {
      }
    }
  ...
  private executeEXT820MISubmitBatch(String CONO, String JOID, String USID, String P001, String P002, String P003) {
    Map<String, String> parameters = ["CONO": CONO, "JOID": JOID, "USID": USID, "P001": P001, "P002": P002, "P003": P003, "EXVSN": "1"]
  ...
  /**
   * Get job data from EXTJOB table
   * @param referenceId
   * @return
   */
